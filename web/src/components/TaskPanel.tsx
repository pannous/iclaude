...
function persistCollapsed(set: Set<string>) {
  try {
    safeStorage.setItem(COLLAPSED_KEY, JSON.stringify([...set]));
  } catch { /* ignore */ }
}
...
function GitHubPRSection({ sessionId }: { sessionId: string }) {
  ...
  if (!prStatus?.available) return null;

  if (prStatus.pr) return <GitHubPRDisplay pr={prStatus.pr} />;

  // No PR yet — show a "Create PR" link when we have a branch and repo slug
  if (!branch || !prStatus.repoSlug) return null;
  const createUrl = `https://github.com/${prStatus.repoSlug}/compare/${encodeURIComponent(branch)}?expand=1`;
  ...
  return (
    <div className="shrink-0 px-4 py-2.5">
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="inline-flex items-center gap-1.5 text-[11px] text-cc-muted hover:text-cc-primary transition-colors"
      >
        <svg viewBox="0 0 16 16" fill="currentColor" className="w-3.5 h-3.5">
          <path fillRule="evenodd" d="M7.177 3.073L9.573.677A.25.25 0 0110 .854v4.792a.25.25 0 01-.427.177L7.177 3.427a.25.25 0 010-.354zM3.75 2.5a.75.75 0 100 1.5.75.75 0 000-1.5zm-2.25.75a2.25 2.25 0 113 2.122v5.256a2.251 2.251 0 11-1.5 0V5.372A2.25 2.25 0 011.5 3.25zM11 2.5h-1V4h1a1 1 0 011 1v5.628a2.251 2.251 0 101.5 0V5A2.5 2.5 0 0011 2.5zm1 10.25a.75.75 0 111.5 0 .75.75 0 01-1.5 0zM3.75 12a.75.75 0 100 1.5.75.75 0 000-1.5z" clipRule="evenodd" />
        </svg>
        Create PR from <span className="font-mono-code">{branch}</span>
      </a>
    </div>
  );
}
