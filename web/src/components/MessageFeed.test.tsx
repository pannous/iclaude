...
describe("MessageFeed - lazy resume transcript", () => {
  ...
  it("shows inline resume banner for active chats and updates loaded transcript status", async () => {
    // LOCAL: For fork sessions, history auto-loads and afterwards shows "Loaded all available prior transcript".
    // The inline banner in the message area shows only after history is loaded (mergedMessages.length > 0
    // from history) while there are still no live session messages (messages.length === 0).
    ...
    // Fork sessions auto-load history without requiring a button click
    await waitFor(() => {
    ...
    // After history loads, shows the "Loaded all available prior transcript" indicator
