...
/**
 * LOCAL: Our McpSection is collapsible and starts collapsed.
 * Call this after render to expand the section so inner content is visible.
 */
function expandSection() {
  // The toggle button contains the text "MCP Servers (N)"
  const toggle = screen.getByText(/^MCP Servers/);
...
describe("McpSection", () => {
  it("renders 'MCP Servers' heading", () => {
    ...
    // LOCAL: our heading shows "MCP Servers (N)" with the count
  ...
  it("shows empty state when no servers are configured", () => {
    ...
    // LOCAL: must expand the collapsible section first
    render(<McpSection sessionId="s1" />);
    expandSection();
...
describe("McpSection add server form", () => {
  ...
  it("hides empty state when add form is open", () => {
    ...
    // LOCAL: must expand first to see the empty state, then click add
    render(<McpSection sessionId="s1" />);
    expandSection();
    expect(screen.getByText(/No MCP servers configured/)).toBeInTheDocument();
...
describe("McpSection server row expansion", () => {
  it("expands to show config details when server name is clicked", () => {
    ...
    // LOCAL: must expand the collapsible section first, then click server
    const servers = [
      makeServer({
        name: "detail-srv",
        status: "connected",
...
describe("McpSection accessibility", () => {
  ...
  it("passes axe accessibility checks with servers", async () => {
    ...
    resetStore({ mcpServers: new Map([["s1", servers]]) });
    const { container } = render(<McpSection sessionId="s1" />);
    expandSection(); // LOCAL: expand to show server list before axe check
    const results = await axe(container);
    expect(results).toHaveNoViolations();
  });
  ...
  it("passes axe accessibility checks with add form open", async () => {
    ...
    resetStore();
    const { container } = render(<McpSection sessionId="s1" />);
    expandSection(); // LOCAL: expand to access add button
    fireEvent.click(screen.getByTitle("Add MCP server"));
    const results = await axe(container);
    expect(results).toHaveNoViolations();
  });
