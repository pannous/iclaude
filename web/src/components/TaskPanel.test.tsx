...
describe("TaskPanel — config mode interactions", () => {
  ...
  it("disables the up arrow on the first section and down arrow on the last", () => {
    ...
    resetStore({ taskPanelConfigMode: true });
    render(<TaskPanel sessionId="s1" />);
    // LOCAL: our section order differs from upstream — "session-cost" is first,
    // "linear-issue" is last for Codex (tasks is claude-only, project-user is claude-only)
    const upFirst = screen.getByTestId("move-up-session-cost");
    expect(upFirst).toBeDisabled();
    const downLast = screen.getByTestId("move-down-linear-issue");
