...
export function HomePage() {
  ...
  // Sandbox state
  const [sandboxEnabled, setSandboxEnabled] = useState(() => safeStorage.getItem("cc-sandbox-enabled") === "true");
  const [sandboxes, setSandboxes] = useState<CompanionSandbox[]>([]);
  const [selectedSandbox, setSelectedSandbox] = useState(() => safeStorage.getItem("cc-selected-sandbox") || "");
  ...
  useEffect(() => {
    ...
    api.listSandboxes().then(setSandboxes).catch((e) => console.warn("[home] listSandboxes", e));
              ...
              <div className="absolute left-0 bottom-full mb-1 w-56 bg-cc-card border border-cc-border rounded-[10px] shadow-lg z-10 py-1 overflow-hidden">
                <button
                  onClick={() => {
                    ...
                    safeStorage.setItem("cc-sandbox-enabled", "false");
                    setShowSandboxDropdown(false);
                  }}
                  className={`w-full px-3 py-2 text-xs text-left hover:bg-cc-hover transition-colors cursor-pointer ${
                ...
                <button
                  onClick={() => {
                    ...
                    safeStorage.setItem("cc-sandbox-enabled", "true");
                    setSelectedSandbox("");
                    safeStorage.setItem("cc-selected-sandbox", "");
                    setShowSandboxDropdown(false);
                  }}
                  className={`w-full px-3 py-2 text-xs text-left hover:bg-cc-hover transition-colors cursor-pointer ${
                ...
                {sandboxes.map((sb) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      safeStorage.setItem("cc-selected-sandbox", sb.slug);
                      setShowSandboxDropdown(false);
                    }}
                    className={`w-full px-3 py-2 text-xs text-left hover:bg-cc-hover transition-colors cursor-pointer flex items-center gap-1 ${
