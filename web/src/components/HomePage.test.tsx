...
describe("HomePage", () => {
  ...
  it("renders the title, logo, textarea, and send button", async () => {
    ...
    // Title
    expect(screen.getByText("iClaude")).toBeInTheDocument();

    // Logo image (the claude logo is the default)
    const logo = screen.getByAltText("iClaude");
  ...
  it("opens and selects from the model dropdown", async () => {
    ...
    // LOCAL: our CLAUDE_MODELS starts with "Default" (value: "default") before Opus 4.6,
    // so the initial displayed model is "Default", not "Opus 4.6" as in upstream.
    render(<HomePage />);
    await screen.findByPlaceholderText("Fix a bug, build a feature, refactor code...");

    // The default model label for claude backend is "Default" (LOCAL: we added "default" option)
    const modelButton = screen.getByText("Default");
    expect(modelButton).toBeInTheDocument();

    // Open model dropdown
    fireEvent.click(modelButton);

    // Should see model options including Sonnet
  ...
  it("creates a session and sends the initial message on submit", async () => {
    ...
    await waitFor(() => {
      expect(createSessionStreamMock).toHaveBeenCalledWith(
        expect.objectContaining({
          // LOCAL: default model is "default" (not "claude-opus-4-6") because we added
          // a "default" option that uses the CLI's configured model.
          model: "default",
  ...
  it("closes model dropdown on outside click", async () => {
    ...
    // LOCAL: default model is "Default" (not "Opus 4.6") because we added a "default" option.
    render(<HomePage />);
    await screen.findByPlaceholderText("Fix a bug, build a feature, refactor code...");
