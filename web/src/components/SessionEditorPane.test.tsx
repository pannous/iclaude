...
interface MockStoreState {
  ...
  // LOCAL: editorActiveFilePath for opening files from chat
...
describe("SessionEditorPane", () => {
  ...
  it("shows large file warning and loads on confirm", async () => {
    ...
    await clickFile("big.json");

    // Warning should appear (editorPanel renders in both desktop & mobile layouts,
    // so we use getAllBy to handle duplicates from responsive layout)
    await waitFor(() => expect(screen.getAllByText(/Large file/).length).toBeGreaterThan(0));
    expect(screen.getAllByText("Open anyway").length).toBeGreaterThan(0);
    // readFile should NOT have been called yet
    expect(readFileMock).not.toHaveBeenCalled();

    // Click "Open anyway" (first instance from desktop layout)
    fireEvent.click(screen.getAllByText("Open anyway")[0]);
  ...
  it("shows error for files exceeding 2 MB limit", async () => {
    ...
    // Click the file to select it
    await clickFile("huge.bin");

    // editorPanel renders in both desktop & mobile layouts — use getAllBy
    await waitFor(() => expect(screen.getAllByText(/too large to edit/i).length).toBeGreaterThan(0));
    expect(readFileMock).not.toHaveBeenCalled();
  });
  ...
  it("close button clears content and resets state", async () => {
    ...
    // Wait for file to load (CodeMirror renders in both desktop & mobile layouts)
    await waitFor(() => expect(readFileMock).toHaveBeenCalled());
    await waitFor(() => expect(screen.getAllByLabelText("Code editor").length).toBeGreaterThan(0));

    // Click close button (desktop layout — first instance)
  ...
  it("displays file size in the header bar", async () => {
    ...
    await waitFor(() => expect(readFileMock).toHaveBeenCalled());
    // editorPanel renders in both desktop & mobile layouts — use findAllBy
    const sizeEls = await screen.findAllByText("2.0 KB");
    expect(sizeEls.length).toBeGreaterThan(0);
  });
  ...
  it("has mobile back button for navigation", async () => {
    ...
    statFileMock.mockResolvedValue({ path: "/repo/main.ts", size: 50, isFile: true });
  ...
  it("fuzzy search filters and selects files from nested directories", async () => {
    ...
    statFileMock.mockResolvedValue({ path: "/repo/src/store.ts", size: 100, isFile: true });
  ...
  it("navigates search results with arrow keys and Enter", async () => {
    ...
    statFileMock.mockResolvedValue({ path: "/repo/beta.ts", size: 50, isFile: true });
