...
beforeEach(() => {
  // Reset modules so the module-level `audioContext` and `hasUserGesture` start fresh
  vi.resetModules();
...
describe("playNotificationSound", () => {
  it("creates an AudioContext and plays two oscillator tones", async () => {
    ...
    const { playNotificationSound, _simulateUserGestureForTest } = await import("./notification-sound.js");
    _simulateUserGestureForTest();
