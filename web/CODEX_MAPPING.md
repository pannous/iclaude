...
This document describes how the Codex `app-server` JSON-RPC protocol maps to the Vibe Companion's internal browser message protocol. The adapter (`web/server/codex-adapter.ts`) performs this translation so the frontend doesn't need to know which backend is running.
...
```
Server → Codex:  {"method":"initialize","id":1,"params":{"clientInfo":{"name":"vibe-companion",...},"capabilities":{}}}
