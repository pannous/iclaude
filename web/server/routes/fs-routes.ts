...
export function registerFsRoutes(api: Hono, opts?: { allowedBases?: string[] }): void {
  ...
  // LOCAL: image route with tilde expansion (used for iMessage image serving)
  ...
  // LOCAL: HTML file proxy route
  api.get("/fs/html", async (c) => {
    const filePath = c.req.query("path");
    if (!filePath) return c.json({ error: "path required" }, 400);
    const expanded = filePath.startsWith("~/")
      ? join(homedir(), filePath.slice(2))
    ...
    if (!/\.html?$/i.test(absPath)) {
      return c.json({ error: "Not an HTML file" }, 400);
    }
    try {
      const file = Bun.file(absPath);
      if (!(await file.exists())) return c.json({ error: "Not found" }, 404);
