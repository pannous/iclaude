...
export class SessionOrchestrator {
  ...
  private async doCreateSession(
    ...
    try {
      ...
      // Pre-load conversation history for resumed sessions so the browser
      // sees it immediately, before system.init arrives from the CLI.
      const resumeCliId = resumeSessionAt || body.resumeSessionId;
      if (resumeCliId) {
        this.wsBridge.initializeResumedSession(session.sessionId, resumeCliId, cwd!);
      }
  ...
  private async handleAutoNaming(sessionId: string, firstUserMessage: string): Promise<void> {
    ...
    if (title && !sessionNames.getName(sessionId)) {
      ...
      this.launcher.setTitle(sessionId, title);
      this.wsBridge.setTitle(sessionId, title);
    }
  }
